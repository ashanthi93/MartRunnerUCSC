...
public class CheckAttendanceThursday extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            /*Start of checkPayment method -- for the week before confirming attendance*/
            paidCheck = vendor.checkPayment("TH");
            ...
            if (paidCheck == 1) {
                ...
                if (expiredDate.compareTo(currentDate) > 0) {
                    ...
                    if (!(dayOfWeek > 2 && dayOfWeek < 6)) {
                        ...
                        if (dayOfWeek == 2 && (compareTimes(closeTime, currentTime) > 0)) {
                            ...
                            String status = vendor.getAttendanceStatus("TH");                      //get current attendance status

                            session.setAttribute("AttendanceT", status);                            //set attendance status to seesion attribute
                    ...
                    session.setAttribute("errorMsg", "Your payment has expired for Thursday Market!");
                    response.sendRedirect("vendor/vendorHomePage.jsp");
                }
            } else if (paidCheck == -1) {
                ...
                session.setAttribute("error", 1);
                session.setAttribute("errorHead", "Invlalid Market Day");
                session.setAttribute("errorMsg", "You are not a memebr of Thursday Market!");
                response.sendRedirect("vendor/vendorHomePage.jsp");
            } else {
